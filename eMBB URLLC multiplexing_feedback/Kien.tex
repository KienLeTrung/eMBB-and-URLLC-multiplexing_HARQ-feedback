...
\begin{abstract}
In uplink (UL) transmission, the Ultra-Reliable Low-Latency Communication (URLLC) users (UEs) due to tight latency requirement might be assigned the grant-free (GF)/configured-grant (CG) resources to transmit data straightaway instead of sending scheduling request (SR) and receiving UL grant. However, the URLLC traffic is sporadic, thus, the base station (called gNB) can dynamically schedule the Enhanced Mobile Broadband (eMBB) UEs to transmit in the GF resources to increase the efficiency of resource utilization. This leads to another concern when a multiplexing of URLLC and eMBB UEs with different requirements in an overlapped time and frequency resource degrades the performance of URLLC and prevents it from attaining the strict requirements of reliability and latency. In this paper, a two-step strategy containing an overlap indication and explicit Hybrid automatic repeat request (HARQ) feedback is proposed to improve URLLC performance in multiplexing with eMBB. Besides the explicit HARQ feedback structure, a scheme with an additional SR is also presented. Simulation results show that these two schemes help to reduce error probability due to Demodulation Reference Signal (DMRS) miss-detection.
